...
// Generate static parameters for all industry pages
export async function generateStaticParams() {
  try {
    const { industries } = await getFilterOptions()
...
// Generate SEO metadata for each industry page
export async function generateMetadata({ params }: IndustryPageProps) {
  const resolvedParams = await params
  const industryName = slugToName(resolvedParams.slug)
  
  try {
    const { industries } = await getFilterOptions()
    const matchingIndustry = industries.find(industry => createSlug(industry) === resolvedParams.slug)
    
    if (!matchingIndustry) {
      return {
        title: 'Industry Not Found | n8n json',
        description: 'The requested industry was not found in our template directory.',
      }
    }
    ...
    return {
      ...
      description: `Discover ${templateCount} n8n automation templates for the ${industryName.toLowerCase()} industry. Browse workflows designed for ${industryName.toLowerCase()} businesses and use cases.`,
      openGraph: {
        title: `${industryName} Templates | n8n json`,
        description: `Discover ${templateCount} n8n automation templates for the ${industryName.toLowerCase()} industry.`,
        url: `https://n8njson.com/industry/${resolvedParams.slug}`,
        type: 'website',
      },
      twitter: {
      ...
      alternates: {
        canonical: `https://n8njson.com/industry/${resolvedParams.slug}`,
      },
    }
  } catch (error) {
    ...
    return {
      title: `${industryName} Templates | n8n json`,
      description: `Browse n8n automation templates for the ${industryName.toLowerCase()} industry.`,
    }
  }
}
          ...
          <div className="text-center">
            ...
            {total > 0 && (
              <p className="text-xl sm:text-2xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
                Discover <span className="text-[#E87C57] font-semibold">{total}</span> automation templates designed for the <span className="text-white font-medium">{industryName.toLowerCase()} industry</span>
              </p>
            )}
        ...
        <div className="mx-auto max-w-6xl">
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center mt-8 gap-2">
              {/* Previous Button */}
              <Link href={currentPage > 1 ? `/industry/${resolvedParams.slug}?page=${currentPage - 1}` : '#'}>
                <Button
              ...
              <div className="flex gap-1 mx-4">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                  ...
                  return (
                    <Link key={page} href={`/industry/${resolvedParams.slug}?page=${page}`}>
                      <Button
              ...
              {/* Next Button */}
              <Link href={currentPage < totalPages ? `/industry/${resolvedParams.slug}?page=${currentPage + 1}` : '#'}>
                <Button
          ...
          {templates.length === 0 && (
            <div className="text-center py-16">
              <p className="text-gray-400 text-lg mb-4">No templates found for the {industryName} industry.</p>
              <Link href="/">
