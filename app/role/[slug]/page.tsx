...
// Generate static parameters for all role pages
export async function generateStaticParams() {
  try {
    const { roles } = await getFilterOptions()
    return roles.map((role) => ({
      slug: createSlug(role),
    }))
  } catch (error) {
    console.error('Error generating static params for roles:', error)
    return []
  }
}

// Generate SEO metadata for each role page
export async function generateMetadata({ params }: RolePageProps) {
  const resolvedParams = await params
  const roleName = slugToName(resolvedParams.slug)
  
  try {
    const { roles } = await getFilterOptions()
    const matchingRole = roles.find(role => createSlug(role) === resolvedParams.slug)
    
    if (!matchingRole) {
      return {
        title: 'Role Not Found | n8n json',
        description: 'The requested role was not found in our template directory.',
      }
    }

    // Use approximate count for SEO metadata (faster than full query)
    const templateCount = 50 // Will be dynamically updated with actual count
        ...
        <div className="mx-auto max-w-6xl">
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center mt-8 gap-2">
              {/* Previous Button */}
              <Link href={currentPage > 1 ? `/role/${resolvedParams.slug}?page=${currentPage - 1}` : '#'}>
                <Button
              ...
              <div className="flex gap-1 mx-4">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                  ...
                  return (
                    <Link key={page} href={`/role/${resolvedParams.slug}?page=${page}`}>
                      <Button
              ...
              {/* Next Button */}
              <Link href={currentPage < totalPages ? `/role/${resolvedParams.slug}?page=${currentPage + 1}` : '#'}>
                <Button
          ...
          {templates.length === 0 && (
            <div className="text-center py-16">
              <p className="text-gray-400 text-lg mb-4">No templates found for the {roleName} role.</p>
              <Link href="/">
